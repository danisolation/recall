...
| 9 | E2E specs leave registered users in the dev database | Dev data accumulates; test fixtures are not self-cleaning | No user-deletion endpoint exists; unique emails keep runs independent | A test-only cleanup helper (direct DB delete in teardown) or a scheduled dev-database reset | Low |
