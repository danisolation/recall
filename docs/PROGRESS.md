...
- Web session state: the home page reads the session server-side (forwarding the browser cookie to `GET /auth/me`) and renders the signed-in user with a logout control, or links to log in / create an account (AUTH-023); `/dashboard` sits behind a `(protected)` route group whose layout redirects unauthenticated visitors to `/login` (AUTH-024).
...
- Playwright E2E in place (AUTH-021): registration, login/logout, and the protected-page redirect all run through the real UI against the real stack (browser → rewrite proxy → API → PostgreSQL) on isolated ports 3100/3101.
...
- Test suites green: API 50 (unit + HTTP/DB integration), web 24 (component), contracts 12 (schema), E2E 4 (Playwright).
- **The auth phase is complete (AUTH-001..024)**: registration, login, sessions, guarded routes, logout, rate limiting, full web UI, and a protected page. The hardening block in `docs/TASKS.md` tracks deferred real-world hardening (registration rate limiting, proxy-aware IP keying, structured logging, security headers, session cleanup).

## Plans for the future

### Now

- The MVP domain work per `docs/ROADMAP.md`: study sets → cards → study sessions → progress → search.
